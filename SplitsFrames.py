...
relevanteStatussen = ["Geplaatst", "Verwijderd"]
outputFile = []
vorigeDatum = ""
vorigeElement = ""

with open('FrameLog.csv', 'rb') as f:
    reader = csv.reader(f)
            ...
            try:
                if logElement[3] == "Status" and logElement[6] in relevanteStatussen:
                    if logElement[0] != vorigeDatum or vorigeElement != row[0]:
                        ...
                        outputRow = [row[0], row[1], datum, logElement[2], logElement[3], logElement[4], logElement[6]]
...
with open("outputFrames.csv", "wb") as f:
    writer = csv.writer(f)
